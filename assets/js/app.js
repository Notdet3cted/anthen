...
(function () {
  ...
  function initGuestFromUrl() {
    ...
    fetch('data/guests.json').then(function (r) { return r.json(); }).then(function (data) {
      var guests = data && data.guests ? data.guests : [];
      var guest = null;
      for (var i = 0; i < guests.length; i++) {
        if (guests[i].slug === slug) { guest = guests[i]; break; }
      }
      if (!guest) {
        guestNameValue = 'Tamu Undangan';
        if (splashName) splashName.textContent = guestNameValue;
        if (hint) hint.textContent = 'Nama tidak ditemukan, silakan hubungi pengundang';
        updateHeroDesc(guestNameValue);
        return;
      }
  ...
  function updateHeroDesc(name) {
    ...
    if (heroDesc) {
      heroDesc.textContent = 'Kepada Yth. ' + name + ', dengan penuh kebahagiaan, kami mengundang Anda untuk menjadi bagian dari momen istimewa dalam kisah cinta kami pada 4 Oktober 2026, serta memberikan doa dan restu bagi perjalanan baru kami bersama.';
    }
  }
  ...
  function toggleMusic() {
    if (bgMusic.paused) {
      bgMusic.play().catch(function () { showToast('Audio tidak tersedia', 'error'); });
    } else {
      bgMusic.pause();
    }
  }
  ...
  function resetGiftSheet() {
    ...
    if (giftSheetSubtitle) giftSheetSubtitle.textContent = 'Pilih cara mengirim hadiah';
  ...
  document.querySelectorAll('.gift-choice').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (giftSheetSubtitle) giftSheetSubtitle.textContent = isBank ? 'Transfer rekening berikut' : 'Kirim kado ke alamat rumah';
      if (giftStepIndicator) giftStepIndicator.textContent = '2/2';
      if (giftDetailTitle) giftDetailTitle.textContent = isBank ? 'Transfer Bank' : 'Kado Fisik';
    });
  });
...
`;
  ...
  storyNext.addEventListener('click', () => {
    if (currentEpIndex < storyData.length - 1) openStoryEpisode(currentEpIndex + 1);
  });
  ...
  let touchStartX = 0;
  storyPages.addEventListener('touchstart', (e) => { touchStartX = e.changedTouches[0].screenX; }, { passive: true });
  storyPages.addEventListener('touchend', (e) => {
